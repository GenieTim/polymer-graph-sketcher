...
import { Drawable, Rectangle, Circle } from "../rendering";
...
export class Application {
  ...
  public renderModeInteractive = new ObservableValue<boolean>(true);
  ...
  render(scaling = { x: 1, y: 1 }, extraElements: Drawable[] = []): void {
    ...
    // Get effective scaling from the ScalingService
    const scalingService = canvasFacade.getScalingService();
    const effectiveScaling = scalingService.getEffectiveScaling(scaling);
    const scalingFactor1D = scalingService.get1DScalingFactor(scaling);
    const canvas = canvasFacade.canvas;

    // Add temporary drawables from current mode if in normal view (not exporting)
    // showSelection.value is false during exports, so temporary elements are excluded
    if (this.renderModeInteractive.value) {
      const modeFactory =
        this.container.get<InteractionModeFactory>("modeFactory");
      const currentMode = modeFactory.getCurrentMode();
      if (currentMode && currentMode.getTemporaryDrawables) {
        const temporaryElements = currentMode.getTemporaryDrawables();
        extraElements = [...extraElements, ...temporaryElements];
      }
    }
    ...
    // Selection circles
    if (this.renderModeInteractive.value && !selection.empty) {
  ...
  destroy(): void {
    ...
    this.renderModeInteractive.clearObservers();
  }
}
